...
@can('expense_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.member-type.create") }}">
                Add
            </a>
        </div>
    </div>
                        ...
                        <tr data-entry-id="{{ $type->id }}">
                            ...
                            <td>
                                @can('expense_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.member-type.edit', $type->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('expense_category_delete')
                                    <form action="{{ route('admin.member-type.destroy', $type->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
